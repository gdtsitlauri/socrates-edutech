...
## AR/VR Educational Module

`src/arvr/` implements a WebXR + Three.js augmented reality quiz layer for SOCRATES:

| File | Role |
|---|---|
| `src/arvr/ar_scene.js` | WebXR session, hit-test, card placement, quiz logic |
| `src/arvr/index.html` | Entry point — desktop or AR launch UI |

### Features

- **AR placement**: marker-less surface detection via WebXR hit-test API; a green reticle ring guides card placement
- **Quiz cards**: 8 CS topic cards (DFA, Big-O, Turing Machines, P vs NP, neural networks, DNS, deadlock, hash functions) rendered as Three.js canvas-texture meshes
- **Card flip interaction**: tap a placed card to reveal the answer (colour + texture update)
- **Spatial anchoring**: each card anchors to a real-world surface pose extracted from the XR reference space
- **Desktop fallback**: when WebXR is unavailable, click the canvas to place cards in 3D desktop mode
- **Responsive**: handles window resize, pixel-ratio scaling, and orientation change

### Running

Serve `src/arvr/` over HTTPS (required for WebXR):

```bash
# e.g. with Python
python3 -m http.server 8443 --directory src/arvr
# or any static HTTPS server
```

Open on an Android device with Chrome and tap **Launch AR**.
