...
\section{AR/VR Educational Interface}
To support immersive, spatial learning experiences, SOCRATES includes a WebXR augmented reality module (\texttt{src/arvr/}) built on Three.js r160 and the WebXR Device API.

\subsection{Scene Design}
The AR session requests the \texttt{immersive-ar} mode with \texttt{hit-test} and \texttt{dom-overlay} required features. A hit-test source is initialised against the \texttt{viewer} reference space each animation frame, and successful surface hits drive a reticle mesh (a green ring geometry) that tracks the detected plane in real time. A desktop fallback places cards at random 3D offsets when WebXR is unavailable.

\subsection{Quiz Card System}
Each tap on a detected surface calls \texttt{placeCard()}, which selects the next card from a fixed rotation of eight CS topics: DFA, Big-O, Turing Machines, P vs NP, neural networks, DNS, deadlock, and hash functions. Every card is realised as a Three.js \texttt{Group} containing a \texttt{BoxGeometry} panel and a \texttt{PlaneGeometry} label carrying a \texttt{CanvasTexture}. The canvas texture is generated by \texttt{makeTextCanvas()} with word wrapping, and updated in place when the learner taps the card to reveal the answer. This avoids geometry re-creation on interaction.

\subsection{Integration}
The AR module is a pure ES-module entry point served as static HTML+JS; it requires no build step and no server-side component. It connects to the SOCRATES-DIALECTIC recommendation layer through the shared JSON schema: a future integration can pre-select which card topic to display based on the learner's current mastery estimate.

\section{Conclusion}
SOCRATES demonstrates a practical research architecture for studying educational adaptation across modern web backends, mobile clients, content pipelines, legacy numerical code, and spatial AR interfaces. The strongest current result remains that causal path optimization can reduce instructional steps without sacrificing mastery, but the more important repository-level outcome is that the complete stack now executes end to end inside the verified workspace. Future work should replace the synthetic encouragement model with classroom telemetry, refine the numerical benchmarking methodology for the legacy bridge, and expand the causal layer with stronger structural assumptions.
